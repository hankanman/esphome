...
static const char *const TAG = "esp32_ble_presense";

ESP32BLEPresense *global_esp32_ble_presense = nullptr;  // NOLINT(cppcoreguidelines-avoid-non-const-global-variables)

float ESP32BLEPresense::get_setup_priority() const { return setup_priority::AFTER_BLUETOOTH; }

void ESP32BLEPresense::setup() {
  if (this->parent_->is_failed()) {
    this->mark_failed();
    ESP_LOGE(TAG, "BLE Presense was marked failed by ESP32BLE");
    return;
  }
  ...
  this->scan_result_buffer_ = allocator.allocate(ESP32BLEPresense::SCAN_RESULT_BUFFER_SIZE);

  if (this->scan_result_buffer_ == nullptr) {
    ESP_LOGE(TAG, "Could not allocate buffer for BLE Presense!");
    this->mark_failed();
  }

  global_esp32_ble_presense = this;
...
void ESP32BLEPresense::loop() {
  ...
  if (!this->scanner_idle_) {
    if (this->scan_result_index_ &&  // if it looks like we have a scan result we will take the lock
      ...
      if (index >= ESP32BLEPresense::SCAN_RESULT_BUFFER_SIZE) {
...
void ESP32BLEPresense::start_scan() {
  if (xSemaphoreTake(this->scan_end_lock_, 0L)) {
    this->start_scan_(true);
  } else {
...
void ESP32BLEPresense::stop_scan() {
...
void ESP32BLEPresense::start_scan_(bool first) {
...
void ESP32BLEPresense::end_of_scan_() {
  // The lock must be held when calling this function.
  if (xSemaphoreTake(this->scan_end_lock_, 0L)) {
...
void ESP32BLEPresense::register_client(ESPBTClient *client) {
  client->app_id = ++this->app_id_;
  this->clients_.push_back(client);
  this->recalculate_advertisement_parser_types();
}

void ESP32BLEPresense::register_listener(ESPBTDeviceListener *listener) {
  listener->set_parent(this);
  this->listeners_.push_back(listener);
  this->recalculate_advertisement_parser_types();
}

void ESP32BLEPresense::recalculate_advertisement_parser_types() {
...
void ESP32BLEPresense::gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param) {
  switch (event) {
...
void ESP32BLEPresense::gap_scan_set_param_complete_(
    const esp_ble_gap_cb_param_t::ble_scan_param_cmpl_evt_param &param) {
  this->scan_set_param_failed_ = param.status;
}

void ESP32BLEPresense::gap_scan_start_complete_(const esp_ble_gap_cb_param_t::ble_scan_start_cmpl_evt_param &param) {
...
void ESP32BLEPresense::gap_scan_stop_complete_(const esp_ble_gap_cb_param_t::ble_scan_stop_cmpl_evt_param &param) {
  xSemaphoreGive(this->scan_end_lock_);
}

void ESP32BLEPresense::gap_scan_result_(const esp_ble_gap_cb_param_t::ble_scan_result_evt_param &param) {
  if (param.search_evt == ESP_GAP_SEARCH_INQ_RES_EVT) {
    if (xSemaphoreTake(this->scan_result_lock_, 0L)) {
      if (this->scan_result_index_ < ESP32BLEPresense::SCAN_RESULT_BUFFER_SIZE) {
...
void ESP32BLEPresense::gattc_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if,
...
void ESP32BLEPresense::dump_config() {
  ESP_LOGCONFIG(TAG, "BLE Presense:");
...
void ESP32BLEPresense::print_bt_device_info(const ESPBTDevice &device) {
